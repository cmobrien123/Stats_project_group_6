...
levels(train$cat_quality) <- c(1,0)
levels(train$cat_quality)


train$Type<-factor(train$Type)
levels(train$Type) <- c(1,0)
levels(train$Type)
...
levels(test$cat_quality) <- c(1,0)
levels(test$cat_quality)


test$Type<-factor(test$Type)
levels(test$Type) <- c(1,0)
levels(test$Type)
...
```{r}
full<-glm(cat_quality~fixed.acidity+volatile.acidity+citric.acid+residual.sugar+total.sulfur.dioxide+density+chlorides+free.sulfur.dioxide+pH+sulphates+alcohol+Type, family=binomial, data=train)
summary(full)
```
...
## removed all insignifigant
reduced_1<-glm(cat_quality~volatile.acidity+residual.sugar+total.sulfur.dioxide+density+free.sulfur.dioxide+pH+sulphates+alcohol+Type, family=binomial, data=train)
summary(reduced_1)
```
...
```{r}
#this time looking at VIF scores
